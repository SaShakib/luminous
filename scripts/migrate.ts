...
async function createPostgresIndexes(): Promise<void> {
  ...
  await dataSource.query("DROP INDEX IF EXISTS idx_orders_user_payment_created_id");
  await dataSource.query("DROP INDEX IF EXISTS idx_orders_user_fulfillment_created_id");
  await dataSource.query("DROP INDEX IF EXISTS idx_orders_user_total_id");
  await dataSource.query("DROP INDEX IF EXISTS idx_order_items_product_sku");
