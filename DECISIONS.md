...
- The first seed was too simple. I changed it to create more realistic users, customer snapshots, addresses, totals, discounts, taxes, shipping, and multiple items per order. I also changed the seed to 500 users and 50,000 orders, spread evenly, so every user has 100 orders and cursor pagination can be tested with the default page size.
...
At 100x, this is about 50,000 users and 5,000,000 orders. That is still a reasonable size for Postgres if the access pattern is indexed, but it is large enough for bad query choices to hurt.
